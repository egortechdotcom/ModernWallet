...
function GenerateSeedPharse({ setWallet, setSeedPharse }) {
  ...
  function setWalletAndMnemonic() {
    ...
    secureLocalStorage.setItem(
      "wallet",
      ethers.Wallet.fromPhrase(newSeedPharse).address
    );
      ...
      <div className="content">
        <div className="mnemonic">
          ...
          <div style={{ fontSize: "14px" }}>
            Once you generate the seed phase, save it securley in order to
        ...
        <Card className="seedPhraseContainer">
          {newSeedPharse && (
            <pre style={{ whiteSpace: "pre-wrap" }}>{newSeedPharse}</pre>
          )}
        </Card>
        <Button
          className="frontPageButton"
...
export default GenerateSeedPharse;
