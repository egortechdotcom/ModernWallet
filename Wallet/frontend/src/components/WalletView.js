...
import { getChromeStorage, removeStorageKey, setChromeStorage } from "../helpers/chromeStorage";
...
function WalletView(
  ...
  const [tokenModalConfirm, settokenModalConfirm] = useState(false);

  const [modalTokenObj, setModalTokenObj] = useState(null)

  const [amountToSendToken, setAmountToSendToken] = useState(null);
  const [sendAddressToken, setSendAddressToken] = useState(null);
  const [processingToken, setProcessingToken] = useState(null);
  const [errorLogToken, setErrorLogToken] = useState(null);
  const [errorMessageTxtToken, setErrorMessageTxtToken] = useState(null);

  const handleTokenModalOk = () => { }
  const handleTokenModalCancel = () => { }

  const sendTransactionToken = (address, amount) => {

  }
  /***
   * token transfer modal end
   */
  ...
  useEffect(() => {
    ...
    getChromeStorage(`${wallet}importedTokens`, (value) => {
  ...
  const handleImportedToken = (item) => {
    setModalTokenObj(item);
    setTokenModal(true);
  }
              ...
              <List
                ...
                renderItem={(item, index) => (
                  <List.Item style={{ textAlign: "left" }}
                    onClick={() => handleImportedToken(item)}
  ...
  async function importToken(address) {
    ...
    let balanceBigint = await contractInstance.balanceOf(address);
    let decimalsBigInt = await contractInstance.decimals();

    let contractData = {
      address: address,
      balance: balanceBigint.toString(),
      decimals: decimalsBigInt.toString(),
    ...
    const existingTokens = getStoredTokensForWallet();

    const updatedTokens = [...existingTokens, tokens];
    setChromeStorage(`${wallet}importedTokens`, JSON.stringify(updatedTokens));
    setContractAddress(null);
  }


  function getStoredTokensForWallet() {
    try {
      let storedTokens = getChromeStorage(`${wallet}importedTokens`);
      if (storedTokens === undefined || storedTokens === null) {
        console.log(`getStoredTokensForWallet ${storedTokens}`);
        return [];
      }
  ...
  async function transferToken(tokenAddress, recipientAddress, amount) {
    ...
    try {
      const amountInWei = ethers.parseUnits(amount.toString(), 18); // Assuming token has 18 decimals
      const tx = await contractInstance.transfer(recipientAddress, amountInWei);
      await tx.wait();
      setTokenModal(false);
      setProcessing(true);
      getAccountToken();
  ...
  function logout() {
    ...
    removeStorageKey(['DseedParse', 'Dwallet']).then(() => {
      navigate('/');
    }).error()
  }
    ...
    <>
      <div className="content">
        ...
        <Modal
          ...
          confirmLoading={tokenModalConfirm}
        ...
        >
          <p style={{ textAlign: 'center' }}>
            {modalTokenObj ?
              (<>
                <h3>
                  {Number(modalTokenObj.balance) / 10 ** Number(modalTokenObj.decimals)}
                  {modalTokenObj.symbol}
                </h3>
                ...
                <div className="sendRow">
                  <p style={{ width: "90px", textAlign: "left" }}>To: </p>
                  <Input
                    value={sendAddressToken}
                    onChange={(e) => setSendAddressToken(e.target.value)}
                    placeholder="0x..."
                  />
                </div>
                <div className="sendRow">
                  <p style={{ width: "90px", textAlign: "left" }}>Amount: </p>
                  <Input
                    value={amountToSendToken}
                    onChange={(e) => setAmountToSendToken(e.target.value)}
                    placeholder="Enter amount to send"
                  />
                </div>
                ...
                {errorLogToken && <p style={{ color: 'red' }}>{errorMessageTxtToken}</p>}


                <Button
                  type="primary"
                  onClick={() => transferToken(modalTokenObj.token_address, sendAddressToken, amountToSendToken)}
                >
                  Send Tokens
                </Button>

              </>)
              : (<>Invalid token selected</>)}

          </p>

        </Modal>

        {/* Imported Token modal end */}
